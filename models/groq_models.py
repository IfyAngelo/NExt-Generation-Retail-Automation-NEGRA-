...
class GroqJSONModel:
    ...
    def invoke(self, messages):
        ...
        try:
            request_response = requests.post(self.model_endpoint, headers=self.headers, data=json.dumps(payload))
            request_response_json = request_response.json()
            if 'choices' not in request_response_json or not request_response_json['choices']:
                raise ValueError("No choices in response")
            response_content = request_response_json['choices'][0]['message']['content']
            response = json.loads(response_content)
            response_formatted = HumanMessage(content=json.dumps(response))
            ...
            error_message = f"Error in invoking model! {str(e)}"
            response_formatted = HumanMessage(content=json.dumps({"error": error_message}))
            return response_formatted
...
class GroqModel:
    ...
    def invoke(self, messages):
        ...
        try:
            ...
            response_formatted = HumanMessage(content=str(response_content))
            return response_formatted
        except Exception as e:
            response_formatted = HumanMessage(content=json.dumps({"error": f"Error in invoking model! {str(e)}"}))
            return response_formatted